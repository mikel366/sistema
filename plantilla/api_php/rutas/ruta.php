...
else
{
    ...
    if(count($rutasArray)== 2 && isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD']=='GET')
    {
        //PETICIONES DE FILTRO PARA MOSTRAR UN SOLO DATO
        if(isset($_GET['linkTo']) && isset($_GET['equalTo']))
        {
            $respuesta = new GetControlador();
            $respuesta->getFiltroData(explode("?", $rutasArray[2])[0], $_GET['linkTo'], $_GET['equalTo']);
        }
        else
        {
            //PETICION PARA TRAER TODA LA INFORMACION
            $respuesta = new GetControlador();
            $respuesta->getData($rutasArray[2]);
        }
    }
}